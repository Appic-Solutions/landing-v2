import { ArrowsUpDownIcon, BlockchainIcon, FireIcon, GovernanceIcon, LockIcon, ParkOutlineBridgeIcon } from "@/components/icon";
...
export const KEY_FEATURES: CardItem[] = [
    {
        ...
        desc: "Appic provides simplified token swaps from any chain to any chain, including ICP and EVM networks. It aggregates liquidity from major DEXs, optimizes swap routes, and completes trades in a single, cost-efficient transaction. Users can easily swap tokens like USDT on Base for ckBTC on ICP or between any supported chains.",
        icon: <BlockchainIcon className="min-w-5 min-h-5" />,
    },
    {
        label: "Appic Bridge",
        desc: "Appic Bridge connects EVM chains to ICP, allowing users to transfer tokens back and forth with maximum speed. It locks tokens on one chain and mints wrapped versions on another, scalable to any EVM network. Soon, it will support direct ICP token bridging to EVMs.",
        icon: <ParkOutlineBridgeIcon className="min-w-5 min-h-5" />,
    },
    {
        label: "Appic Dex",
        desc: "Appic Dex offers fast, efficient token swaps on ICP using concentrated liquidity pools. Liquidity providers can target specific price ranges for better returns, while traders enjoy low slippage and fees. All swaps happen in one canister for atomic speed.",
        icon: <ArrowsUpDownIcon className="min-w-5 min-h-5" />,
    },
    {
        label: "Appic Twin Token Creator",
        desc: `Appic Twin Token Creator lets projects create wrapped "twin" tokens on ICP or EVM chains permissionlessly. It enables seamless bridging of these tokens via Appic Bridge, connecting ecosystems. For example, Gold token on ICP can expand to Base or Arbitrum instantly.`,
